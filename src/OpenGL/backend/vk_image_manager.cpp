...
OpenGL::VKImageReferenceUniquePtr OpenGL::VKImageManager::acquire_object(const GLuint&      in_id,
                                                                           ...
                                                                           OpenGL::TimeMarker in_time_marker)
{
    FUN_ENTRY(DEBUG_DEPTH);
    ...
    vkgl_assert(in_time_marker != OpenGL::LATEST_SNAPSHOT_AVAILABLE); /* This special value is only allowed for frontend managers ! */

    image_data_ptr = image_props_iterator->second.get();

    {
        auto image_iterator 			= image_data_ptr->image_map.find(in_time_marker);
        ...
        switch (frontend_object_type)
        {
            case OpenGL::ObjectType::Texture:
            {
                ...
                else
                {
                    ...
                    image_data_ptr->image_map[in_time_marker].reset(
                    ...
                    vkgl_assert(image_data_ptr->image_map[in_time_marker] != nullptr);
        
                    image_data_ptr->tot_image_time_marker = in_time_marker;
                }
